package com.example.mrchatbot.fragment;
...
import com.example.mrchatbot.activity.SplashActivity;
...
public class ProfileFragment extends Fragment {
    ...
    void updateBtnClick(){
        ...
        if (newUsername.isEmpty() || newUsername.length() < 3) {
            usernameInput.setError("Length should be greater than 3");
            return;
        }
        ...
        if(selectedImageUri!=null){
            FirebaseUtils.getCurrentProfilePicStorage().putFile(selectedImageUri)
    ...
    void getUserData(){
        setInProgress(true);
        FirebaseUtils.getCurrentProfilePicStorage().getDownloadUrl()
                        .addOnCompleteListener(task -> {
                            if(task.isSuccessful()){
